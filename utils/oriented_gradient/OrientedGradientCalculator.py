...
class OrientedGradientCalculator(RotationAdapter):
    ...
    def _operate(self):
        ...
        for i in range(self.radius, rows - self.radius + 1):
            for j in range(self.radius, cols - self.radius + 1):
                output[i, j] = self._calculate_single_oriented_gradient(i,j)
    ...
    def _calculate_single_oriented_gradient(self, row, col):
        row_left = 0 if row - self.radius <= 0 else row - self.radius
